    ...
    validateUser(req.body.username).then( async isvalid => {
        if(!isvalid){
            ...
            try {
                ...
                const result = await contract.submitTransaction('harvest', 'Producer', req.body.weight, req.body.location, req.body.producerID, timestamp)
    ...
    validateUser(req.body.username).then( async isvalid => {
        if(!isvalid){
            ...
            try {
                ...
                const contract = network.getContract('gof');
                //harvest(ctx, role, weight, loc, producerId,timestamp)
                const result = await contract.evaluateTransaction('queryBox','1')
                debugger
